  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <Button variant="primary" icon={Plus} onClick={() => addPavedArea()} title={t('Add a paved floor under the roof plan')}>
          {t('Add floor')}
        </Button>
        <p className="text-[11px] text-slate-500">{t('Drag the slab to move it, blue corners to reshape, green edge handles move a whole side (Alt-drag adds a corner).')}</p>
      </div>

      {areas.length > 0 && (
        <dl className="grid grid-cols-3 gap-2 rounded-md border border-slate-800 bg-slate-900/60 p-2 text-[11px]">
          <div>
            <dt className="text-slate-500">{t('Total base area')}</dt>
            <dd className="font-mono text-slate-200">{paving.totalAreaM2.toFixed(2)} m²</dd>
          </div>
          <div>
            <dt className="text-slate-500">{t('Floors')}</dt>
            <dd className="font-mono text-slate-200">{areas.length}</dd>
          </div>
          <div>
            <dt className="text-slate-500">{t('Stones ≈')}</dt>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                aria-label={t('Floor name')}
              />
              <span className="font-mono text-[11px] text-slate-300">{summary?.areaM2.toFixed(2)} m²</span>
              <button type="button" title={t('Remove floor')} className="rounded p-1 text-rose-300 hover:bg-rose-950/60" onClick={(e) => { e.stopPropagation(); removePavedArea(a.id); }}>
                <Trash2 className="h-4 w-4" />
              </button>
            </div>
            {summary?.selfIntersecting && <p className="text-[11px] text-rose-300">{t('Edges cross each other – untangle the corners, the area figure is not reliable.')}</p>}

            <div className="grid grid-cols-2 gap-2">
              <NumberField label={t('Extent X')} value={width} min={100} step={100} range={[100, 30000]} compact onChange={(v) => resizePavedArea(a.id, v, depth)} />
              <NumberField label={t('Extent Z')} value={depth} min={100} step={100} range={[100, 30000]} compact onChange={(v) => resizePavedArea(a.id, width, v)} />
              <NumberField label={t('Origin X')} value={summary?.bounds.minX ?? 0} step={50} compact onChange={(v) => translatePavedArea(a.id, v - (summary?.bounds.minX ?? 0), 0)} />
              <NumberField label={t('Origin Z')} value={summary?.bounds.minZ ?? 0} step={50} compact onChange={(v) => translatePavedArea(a.id, 0, v - (summary?.bounds.minZ ?? 0))} />
            </div>

            {selected && (
              <>
                <SelectField<PavingPattern> label={t('Pattern')} value={a.pattern} onChange={(pattern) => updatePavedArea(a.id, { pattern })} options={PAVING_PATTERNS.map((p) => ({ value: p.id, label: t(p.label) }))} />
                <div className="grid grid-cols-2 gap-2">
                  <NumberField label={t('Stone length')} value={a.stoneLength} min={40} max={1000} step={10} compact onChange={(stoneLength) => updatePavedArea(a.id, { stoneLength })} />
                  <NumberField label={t('Stone width')} value={a.stoneWidth} min={40} max={1000} step={10} compact onChange={(stoneWidth) => updatePavedArea(a.id, { stoneWidth })} />
                  <NumberField label={t('Joint')} value={a.jointWidth} min={0} max={30} step={1} compact onChange={(jointWidth) => updatePavedArea(a.id, { jointWidth })} />
                  <NumberField label={t('Thickness')} value={a.stoneThickness} min={20} max={200} step={10} compact onChange={(stoneThickness) => updatePavedArea(a.id, { stoneThickness })} />
                </div>
                ...
                {summary && (
                  <p className="text-[11px] text-slate-500">
                    {t('Perimeter {p} m · ≈ {n} stones ({l}×{w} mm, joint {j} mm) · {v} m³ stone.', {
                      p: summary.perimeterM.toFixed(2),
                      n: summary.stoneCount,
                      l: a.stoneLength,
                      w: a.stoneWidth,
                      j: a.jointWidth,
                      v: (summary.areaM2 * a.stoneThickness / 1000).toFixed(2),
                    })}
                  </p>
                )}
                ...
                <div className="space-y-1">
                  <div className="flex items-baseline justify-between text-[11px] font-medium tracking-wide text-slate-400 uppercase">
                    <span>{t('Corners ({n})', { n: a.points.length })}</span>
                    <span className="text-[10px] font-normal normal-case text-slate-500">{t('arrow keys nudge · Delete removes')}</span>
                  </div>
                  {a.points.map((p, i) => (
                    <div
                      key={i}
                    ...
                    >
                      ...
                      <div className="grid min-w-0 flex-1 grid-cols-2 gap-1">
                        <NumberField label={t('X')} value={p.x} step={50} compact onChange={(x) => movePavedPoint(a.id, i, { x, z: p.z })} />
                        <NumberField label={t('Z')} value={p.z} step={50} compact onChange={(z) => movePavedPoint(a.id, i, { x: p.x, z })} />
                      </div>
                      <button type="button" title={t('Add a corner after this one')} className="rounded p-1 text-lime-300 hover:bg-slate-800" onClick={(e) => { e.stopPropagation(); insertPavedPoint(a.id, i); }}>
                        <CornerDownRight className="h-3.5 w-3.5" />
                      </button>
                      <button
                        type="button"
                        title={a.points.length > 3 ? t('Remove corner') : t('A floor needs at least 3 corners')}
