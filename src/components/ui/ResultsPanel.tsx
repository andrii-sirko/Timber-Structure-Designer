        ...
        {tab === "bom" && (
          <div>
            <table className="w-full border-collapse">
              <thead className="sticky top-0 bg-slate-950">
                <tr>
                  ...
                  <th className={cx(th, "text-right")}>Pcs</th>
                  <th className={cx(th, "text-right")}>Length</th>
            ...
            <div className="grid grid-cols-3 gap-2 p-3 text-[11px]">
              <Stat label="Mass" value={`${Math.round(bom.totalMassKg)} kg`} />
              <Stat
                label="Roof area"
          ...
          <div>
            <div className="flex gap-2 border-b border-slate-800 p-2">
              ...
              <span className="ml-auto self-center text-[11px] text-slate-500">
                {cutList.length} positions ·{" "}
                {cutList.reduce((s, c) => s + c.quantity, 0)} pieces
              </span>
            </div>
            <table className="w-full border-collapse">
              <thead className="sticky top-0 bg-slate-950">
                <tr>
                  ...
                  <th className={th}>Section</th>
                  <th className={cx(th, "text-right")}>Length</th>
                  <th className={cx(th, "text-right")}>Cuts</th>
          ...
          <div className="space-y-2 p-2">
            ...
            >
              <span className="flex-1">
                Overall: {STATUS_LABEL[statics.status]}
              </span>
              {statics.status !== "ok" && (
                <Button
                  size="sm"
                  ...
                  icon={Wand2}
                  onClick={runAutoFix}
                  title="Step up sections / add posts until every check passes"
                >
                  Auto-fix all
                </Button>
              )}
              {statics.status === "ok" && (
                <Button
                  ...
                  title="Reduce timber sections while keeping every statics check sufficient"
                >
                  Cost optimization
                </Button>
              )}
              ...
              >
                  ...
                  <span className="flex-1 font-medium text-slate-100">
                    {autoFixReport.action === "cost-optimization"
                      ? autoFixReport.changes.length === 0
                        ? "Cost optimization: no smaller sections pass all checks."
                        : `Cost optimization applied ${autoFixReport.changes.length} change${autoFixReport.changes.length === 1 ? "" : "s"} – all checks pass.`
                      : autoFixReport.changes.length === 0
                        ? "Auto-fix: nothing to change."
                        : autoFixReport.status === "ok"
                          ? `Auto-fix applied ${autoFixReport.changes.length} change${autoFixReport.changes.length === 1 ? "" : "s"} – all checks pass.`
                          : `Auto-fix applied ${autoFixReport.changes.length} change${autoFixReport.changes.length === 1 ? "" : "s"} – some checks still need attention.`}
                  </span>
                  <button
                    type="button"
                    ...
                    title="Dismiss"
                    aria-label="Dismiss statics report"
                  >
                ...
                {autoFixReport.unresolved.length > 0 && (
                  <p className="mt-1 text-amber-200">
                    Still not ok: {autoFixReport.unresolved.join(", ")} – reduce
                    spans, openings or loads manually.
                  </p>
                )}
              </div>
            )}
            ...
            {statics.checks.map((c) => (
              ...
              >
                ...
                <div className="mt-1 grid grid-cols-4 gap-1 text-[11px]">
                  ...
                  />
                  <Stat label="Span" value={`${c.span} mm`} />
                  <Stat
                    label="Stress"
          ...
          <div className="space-y-1.5 p-2">
            {framing.warnings.length === 0 && (
              <p className="text-xs text-emerald-300">
                No geometry or framing issues detected.
              </p>
            )}
            {vehicles.length > 0 && (
