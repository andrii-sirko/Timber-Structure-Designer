...
test('dragging the left / rear wall sets the dimension to the pointer position, snapped to 10 mm', () => {
  assert.equal(wallDragDimensionValue('left', { x: 7024, z: 100 }, params), 7020);
  assert.equal(wallDragDimensionValue('rear', { x: 100, z: 3486 }, params), 3490);
});

test('dragging the right / front wall outwards grows the footprint', () => {
  assert.equal(wallDragDimensionValue('right', { x: -500, z: 0 }, params), 6500);
  assert.equal(wallDragDimensionValue('front', { x: 0, z: 400 }, params), 2600);
});

test('never shrinks below the minimum plan dimension', () => {
  assert.equal(wallDragDimensionValue('left', { x: -3000, z: 0 }, params), 500);
  assert.equal(wallDragDimensionValue('right', { x: 9000, z: 0 }, params), 500);
});

test('grab offset keeps the wall from jumping to the pointer', () => {
  const grab = { x: 6250, z: 1500 };
  const offset = wallDragGrabOffset('left', grab, params);
  assert.equal(offset, 250);
  assert.equal(wallDragDimensionValue('left', applyGrabOffset('left', grab, offset), params), 6000);
  const grabRight = { x: -250, z: 1500 };
  const offsetRight = wallDragGrabOffset('right', grabRight, params);
  assert.equal(wallDragDimensionValue('right', applyGrabOffset('right', grabRight, offsetRight), params), 6000);
});

test('handles sit just outside each wall face', () => {
  const handles = wallHandles(params, 250);
  assert.deepEqual(handles.find((h) => h.wall === 'left')?.centre, { x: 6250, z: 1500 });
  assert.deepEqual(handles.find((h) => h.wall === 'right')?.centre, { x: -250, z: 1500 });
